...
local function find(query)
  ...
  query = query or ""
  local fff_result =
    file_picker.search_files(query, state.current_file_cache, 100, 4, nil)
  ...
  for _, fff_item in ipairs(fff_result) do
    local item = {
      ...
      path = fff_item.path,
      score = fff_item.total_frecency_score or 0,
    }
...
return function()
  ...
  require("minibuffer").select({
    ...
    format_fn = function(item)
      ...
      local dir = vim.fn.fnamemodify(item.text, ":h")
    ...
    on_start = function(buf, sess, keyset)
      ...
      keyset("i", "<C-q>", function()
        ...
        for _, i in ipairs(indices) do
          ...
          if file then
            qf[#qf + 1] = { filename = file.path, lnum = 1, col = 1, text = file.text }
          end
        end
        if #qf > 0 then
